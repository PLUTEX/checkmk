...
from typing import Any, cast, Dict, Generator, List, Optional, Pattern, Set, Tuple, TYPE_CHECKING, FrozenSet, Iterable
...
class RulesetMatcher:
    ...
    def get_host_ruleset_values(
        ...
        optimized_ruleset = self.ruleset_optimizer.get_host_ruleset(
            ruleset, with_foreign_hosts, is_binary=is_binary, limit=frozenset(
...
class RulesetOptimizer:
    ...
    def get_host_ruleset(
        ...
        limit: Optional[FrozenSet[str]] = None
    ...
    def _convert_service_ruleset(
        ...
        for rule in ruleset:
            ...
            # recomputation later
            hosts = self._all_matching_hosts(rule["condition"], with_foreign_hosts, limit)
    ...
    def _all_matching_hosts(
        ...
        if hostlist == []:
            ...
            if len(hosts_to_check) > 1:
                ...
                with open("/tmp/debug.log", mode='a', encoding='utf-8') as f:
                    traceback.print_stack(file=f)
                    print(condition, limit, file=f)
